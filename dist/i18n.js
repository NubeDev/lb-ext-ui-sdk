 ...
 */
export function resolveLocale(userPref, enabled = [FALLBACK_LOCALE]) {
    if (userPref && enabled.includes(userPref))
        return userPref;
    const nav = typeof navigator !== "undefined" && navigator.language
        ? navigator.language.split("-")[0]
        : "";
    if (nav && enabled.includes(nav))
        return nav;
    return FALLBACK_LOCALE;
}
/** Interpolate `{name}` placeholders from `args`. A missing arg renders `[name]` (never blank). */
function interpolate(src, args) {
    return src.replace(/\{(\w+)\}/g, (_, name) => args && name in args ? String(args[name]) : `[${name}]`);
}
/**
 ...
 */
export function makeTranslator(catalogs, locale) {
    return (key, args) => {
 ...
 */
export function catalogParity(catalogs) {
    const locales = Object.keys(catalogs);
    if (locales.length === 0)
        return ["no catalogs registered"];
    ...
    for (const loc of locales.slice(1)) {
        ...
        for (const k of baseKeys)
            if (!keys.has(k))
                problems.push(`${loc}: missing key '${k}' (present in ${base})`);
        for (const k of keys)
            if (!baseKeys.has(k))
                problems.push(`${base}: missing key '${k}' (present in ${loc})`);
    }
    return problems;
}
